...
**Fix**: Explicitly state the full date or date range in your prompt before using relative terms like "this month" or "last quarter". Once the agent has the current date in context, relative terms typically return desired results.
...
**Cause**: The project definition schema is complex, and some models might produce malformed JSON (especially in deeply nested structures).
